...
import hs_bridge
import MLP_2000_1000hidden, MLP_128hidden
import torch
import torchvision
...
model_file = MLP_2000_1000hidden
...
model_arch = model_file.LinearModel(10)
conv_layers, linear_layers = _get_model_layers(model_arch)
...
extra_timesteps = len(conv_layers) + len(linear_layers)

#parameters
batch_size = 64

#weights path
PATH = "/home/k7arora/hs_api/examples/CRI_Mapping/chris_code/CIFAR10_SNN_Finetuning/Final_SNN_Model/MLP_weights_2000_1000hidden"
T=30
...
def main():
    ...
    model = model_file.LinearModel(10).to(device)
    ...
    if len(conv_layers) > 0:
        ...
        for conv_idx in range(1, len(conv_layers)):
            ...
            CMap = torch.arange(prev_res ** 2, dtype=torch.float32).reshape(1, 1, prev_res, prev_res)
            patchTensor = F.unfold(
                input=CMap,
            ...
            for output_idx, output_channel in enumerate(int16_sd[conv_weight_names[conv_idx]]):
                for feature_map, kernel in enumerate(output_channel):
                    ...
                    for j, row in enumerate(patch_rows):
                        ...
                        for i, elem in enumerate(row):
                            index = int(elem.item())
                            key = f"C{conv_idx}.{feature_map}.{index}"
                            weight = flat_kernel[i].item()
        ...
        if len(linear_weight_names) > 0:
            ...
            conv_last_res2 = conv_output_res_list[-1] ** 2
            for col in range(int16_sd[linear_weight_names[0]].shape[1]):
                if col % conv_last_res2 == 0 and col != 0:
                    feature_map += 1
                sourceNeuron = f"C{len(conv_layers)}.{feature_map}.{col % conv_last_res2}"
    ...
    model_name = model_file.__name__
    mlp_variant = "mlp_4layers" if model_name == "MLP_2000_1000hidden" else "mlp_3layers"
    dict_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "dictionaries", mlp_variant, "modular")
